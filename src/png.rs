...
impl TryFrom<&[u8]> for Png{
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        if value.len()<8{
            return Err(PngError::InValidPngHeader)
        }
        let (header,chunks_bytes)=value.split_at(8);
        if header.to_vec()!=Png::STANDARD_HEADERS{
            return Err(PngError::ChunkHeaderError);
        }
        let mut chunks_vec:Vec<Chunk>=Vec::new();
        let mut index=0;
        while index<chunks_bytes.len(){
            if chunks_bytes.len()<12{
                return Err(PngError::ChunkError);
            }
            let chunk_len_bytes:[u8;4]=chunks_bytes[index..index+4].try_into().expect("failed in png.rs try_from()");
            let chunk_len=u32::from_be_bytes(chunk_len_bytes);
            let chunk_end=index+4+4+chunk_len as usize + 4;//chunk类型：长度 4字节，类型4字节，数据长度：chunk_len，crc：4字节
            if chunk_end>chunks_bytes.len(){
                return Err(PngError::ChunkError);
            }
            let chunk_bytes=&chunks_bytes[index..chunk_end];
            let chunk=Chunk::try_from(chunk_bytes)?;
            chunks_vec.push(chunk);
            index=chunk_end;
        }
        Ok(Png { signature: header.try_into().unwrap(), chunks: chunks_vec })
...
impl Png{
    const STANDARD_HEADERS:[u8;8]=[137,80,78,71,13,10,26,10];
    pub fn from_chunks(chunks:Vec<Chunk>)->Png{
        Png { signature:Png::STANDARD_HEADERS, chunks: chunks }
    }
    ...
    pub fn remove_chunk(&mut self,chunk_type:&str)->Result<Chunk,PngError>{
        let index=self.chunks
            ...
            .ok_or(PngError::NotFoundChunkType);
    ...
    pub fn as_bytes(&self)->Vec<u8>{
        let mut vec=self.signature.to_vec();
        for chunk in &self.chunks{
            let chunk_bytes=chunk.as_bytes();
            vec.extend(chunk_bytes);
        }
        vec
    }
}
...
mod tests {
    ...
    fn test_valid_from_bytes() {
        ...
        let bytes: Vec<u8> = Png::STANDARD_HEADERS
            .iter()
